...
@section('content')
    ...
    <main id="main" class="main setting">
        ...
        <div class="main_content">
            <form action="{{url('api/v1/tournament/match-link/create')}}/{{$pagetId}}" id="form" method="post"
                        ...
                        <div class="row align-items-end">
                            <div class="col-lg-3">
                                ...
                                <select class="form-select" name="link_type" id="link_type">
                                    <option selected disabled>Match Live Link</option>
                                    <option value="youtube">Youtube</option>
                                    <option value="mp4">MP4</option>
                                    <option value="m3u8">M3U8</option>
                                    <!-- <option value="3">Pakistan</option> -->
                                </select>
                                <div class="text-danger mb-4 link_type_error" id=""></div>
                            </div>
...
@push('custom-js')
    <script>
        ...
        $.ajax({
            url: window.origin + '/api/v1/tournament/get-match-schedule/' + id,
            type: 'GET',
            dataType: "json",
                ...
                if (res.status === "success") {
                   ...
                   `)
                    ...
                    <div class="d-flex align-items-center text-dark mb-4">
                        ...
                        <div class="mb-4">
                            <input type="text" class="form-control" placeholder="Link Here" name="first_team_couch"
                                   value="${res.data.first_team.couch_name}">
                        </div>
                    ...
                    res.data.first_team.players.forEach(function (playersData) {
                      ...
                      my-3
                    ">
                           <input type="checkbox" class="mt-0" name="first_squad[]" value='{"name":"${playersData.name}","role":"${playersData.role}"}'>
                    ...
                    <div class="d-flex align-items-center text-dark mb-4">
                        ...
                        <div class="mb-4">
                            <input type="text" class="form-control" placeholder="Link Here" name="second_team_couch"
                                   value="${res.data.second_team.couch_name}">
                        </div>
                        <label for="" class="mt-2">Select Captain</label>
                    ...
                    res.data.second_team.players.forEach(function (playersData) {
                        $("#secondTeamPlayersList").append(`
                            <div class="
                              ...
                              my-3
                            ">
                            <input type="checkbox" class="mt-0" name="second_squad[]" value='{"name":"${playersData.name}","role":"${playersData.role}"}'>
                            <h6 class="mx-4">${playersData.name}</h6>
                               <p>${(playersData.role)!==null?playersData.role:"No role selected"}</p>
                        </div>
                    `)
